...
int main(int argc, char **arv) {
  ...
  int groundLocationsSize = 10;
  int groundLocationsIndex = 0;
  int *groundLocations = malloc(sizeof(int) * groundLocationsSize);
  ...
  while ((c = getchar()) != EOF) {
    ...
    if (c == 'a') {
      groundLocations[groundLocationsIndex++] = counter;
      if (groundLocationsIndex == groundLocationsSize) {
        groundLocationsSize *= 2;
        groundLocations =
            realloc(groundLocations, sizeof(int) * groundLocationsSize);
      }
      groundLocations[groundLocationsIndex] = -1;
    }
    map[counter++] = c;
  }
  ...
  for (int i = 0;; i++) {
    int gl = groundLocations[i];
    if (gl == -1) {
      break;
    }
    int dist = distance[gl];
    if (dist < shortest) {
      shortest = distance[gl];
    }
  }
  printf("%d\n", shortest);
}